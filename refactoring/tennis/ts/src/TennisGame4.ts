...
export class TennisGame4 implements TennisGame {
    ...
    let result = new Deuce(
      ...
      new GameServer(
        ...
        new GameReceiver(
          ...
          new AdvantageServer(
            this,
            new AdvantageReceiver(this, new DefaultResult(this)),
          ),
        ),
      ),
    ).getResult();
  ...
  receiverHasWon() {
    return (
      this.receiverScore >= 4 && this.receiverScore - this.serverScore >= 2
    );
  }
  ...
  wonPoint(playerName: string): void {
    if (playerName === 'player1') this.serverScore += 1;
    else this.receiverScore += 1;
  }
}
...
class DefaultResult {
  ...
  getResult() {
    return new TennisResult(
      this.scores[this.game.serverScore],
      this.scores[this.game.receiverScore],
    );
  }
}
